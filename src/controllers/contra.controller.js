...
exports.createContra = async (req, res) => {
  ...
  try {
    ...
    if (!data.entries || !data.entries.length) {
      throw new Error(
        "At least one contra entry is required"
      );
    }

    // Validate credit/account ledger
    const isAccountValid =
      await contraModel.validateContraLedger(
        connection,
        data.account_ledger_id
      );

    if (!isAccountValid) {
      throw new Error(
        "Invalid account ledger selected"
      );
    }
    ...
    const contraId =
      await contraModel.createContra(connection, {
        voucher_type_id,
        voucher_no,
    ...
    for (const entry of data.entries) {
      ...
      // Save voucher entry
      await contraModel.insertContraEntry(
        connection,
        contraId,
        {
      ...
      await contraModel.insertLedgerTransaction(
        connection,
        {
...
exports.getContraInvoice = async (req, res) => {
    try {
        const { id } = req.params;
        const data = await contraModel.getContraInvoice(id);
        ...
        console.error( "Get Contra Invoice Error:", error );
        return res.status(500).json({
            success: false,
            message: error.message
        });
