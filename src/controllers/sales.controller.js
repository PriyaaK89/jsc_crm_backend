...
if (!data.customer_ledger_id) {
  throw new Error("Customer ledger is required");
    ...
    data.extra_ledgers = JSON.parse(data.extra_ledgers || "[]").filter(
      (ledger) =>
        ledger.ledger_id !== "" &&
    ...
    for (const item of data.items) {
      ...
      if (Number(availableStock) < Number(item.billed_qty)) {
        throw new Error(
          `Insufficient stock for Item ID ${item.stock_item_id}. Available: ${availableStock}, Required: ${item.billed_qty}`,
        );
      }
    }
...
console.log("DEBUG total_amount:", data.total_amount, typeof data.total_amount);
    ...
    for (const item of data.items) {
      const salesItemId = await salesModel.insertSalesItem(
        connection,
        item,
        saleId,
      );
      if (item.batch_no) {
        await salesModel.insertSalesBatch(connection, item, salesItemId);
      }
    ...
    if (customerLedger?.maintain_bill_by_bill) {
      const creditDays = Number(customerLedger.default_credit_period || 0);
    //   const dueDate = salesModel.addDays(data.sales_date, creditDays);
      const dueDate = new Date(data.sales_date);
...
exports.checkLedgerOverdueStatus = async (req, res) => {
  const connection = await db.getConnection();
  try {
    const { ledgerId } = req.params;
    const salesDate = req.query.sales_date || new Date().toISOString().slice(0, 10);

    if (!ledgerId) {
      return res.status(400).json({ success: false, message: "Ledger ID is required" });
    }

    const overdueBills = await salesModel.checkOverdueBills(connection, ledgerId, salesDate);

    return res.status(200).json({
      success: true,
      has_overdue: overdueBills.length > 0,
