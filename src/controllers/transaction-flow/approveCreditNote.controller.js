...
exports.approveCreditNoteOrder = async (req, res) => {
  ...
  try {
    ...
    let employeeMessage = "";
    let approverMessage = "";
    let seniorApprovalResult = null; // ← hoisted so it's visible after the if/else chain
    ...
    if (approval.approval_level === "JUNIOR") {
      ...
      seniorApprovalResult = result; // ← store it so it survives out of this block
    ...
    const responsePayload = { success: true, message: "Approved successfully" };

    if (seniorApprovalResult) {
      responsePayload.credit_note_id = seniorApprovalResult.creditNoteId;
      responsePayload.voucher_no = seniorApprovalResult.voucher_no;
    }
