...
const purchaseApprovalService = require("../../services/purchase/createPurchaseTransaction.service");
...
exports.approvePurchaseOrder = async (req, res) => {
  ...
  try {
    ...
    if (req.files?.dispatch_doc_image?.[0]) {
      const uploaded = await uploadFileToMinio(req.files.dispatch_doc_image[0], "txn_purchase");
      newPayload.dispatch_doc_image = uploaded.object_path;
    }

    if (req.files?.bill_t_image?.[0]) {
      const uploaded = await uploadFileToMinio(req.files.bill_t_image[0], "txn_purchase");
    ...
    if (approval.approval_level === "JUNIOR") {
      ...
      employeeMessage = `Purchase Order approved by ${approval.junior_name} (Junior Accountant). Pending at ${approval.dispatcher_name} (Dispatcher).`;
      approverMessage = `Purchase Order approved by Junior Accountant (${approval.junior_name}), order generated by ${employeeName}. Pending for your approval.`;
      ...
      nextLevel = "SENIOR";
      employeeMessage = `Purchase Order approved by ${approval.dispatcher_name} (Dispatcher). Pending at ${approval.senior_name}.`;
      approverMessage = `Purchase Order approved by Dispatcher (${approval.dispatcher_name}), order generated by ${employeeName}. Pending for your approval.`;
      ...
      const result = await purchaseApprovalService.executeApprovedPurchase(connection, newPayload, approval.created_by);
      ...
      await transactionApprovalModel.updateApproval(connection, approval.id, {
        ...
        remarks,
        current_status_message: `Approved. Purchase Voucher ${result.voucher_no} generated`,
        approved_at: new Date(),
        final_transaction_id: result.purchaseId,
      });

      employeeMessage = `Purchase Order approved by ${approval.senior_name} (Senior Accountant). Purchase Voucher ${result.voucher_no} generated successfully.`;
      await transactionApprovalModel.updateStatusNotification(connection, approvalId, employeeMessage);
    }
