...
exports.createTarget = async (values) => {
  const query = `
        ...
        ( assigned_by, assigned_to, category_id, visit_type, target_type, target_value, start_date, end_date )
        VALUES ? `;
...
exports.getTargets = async (userId) => {
  const [rows] = await db.query(
    `
        ...
        LEFT JOIN target_categories tc
        ON tc.id=vt.category_id
...
exports.getVisitStats = async (userIds, type) => {
  ...
  switch (type) {
    ...
    case "MONTHLY":
      dateFilter =
        "MONTH(v.created_at) = MONTH(CURDATE()) AND YEAR(v.created_at) = YEAR(CURDATE())";
      break;
