...
exports.executeApprovedReceipt = async (connection, payload, createdBy) => {
  ...
  const receiptData = { ...payload, voucher_no, voucher_type_id, created_by: createdBy };
  ...
  for (const entry of entries) {
    ...
    // ── CHANGED: split AGST vs non-AGST, same as the direct receipt controller ──
    ...
    if (billRefs.length > 0) {
      ...
      // Locks the sales_bill_references rows, re-validates pending amount at
      // approval time (not creation time), and inserts the AGST rows.
      ...
      // ON ACCOUNT / ADVANCE rows: save only, no pending-amount deduction
  ...
  await paymentModal.insertLedgerTransaction(connection, {
    ...
    entry_type: "Dr",
    amount: Number(receiptData.total_amount),
    remarks: receiptData.narration,
    created_by: createdBy,
  });
